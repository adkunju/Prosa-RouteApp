...
WITH lines AS (
  ...
  SELECT delivery_line_id, sum(qty_returned) AS ret
  FROM returns WHERE delivery_line_id IS NOT NULL GROUP BY delivery_line_id
), visits AS (
  SELECT l.store_id, l.sku_id, l.delivered_on,
         sum(l.qty_delivered)::integer AS q, sum(COALESCE(rr.ret, 0))::integer AS ret
  FROM lines l LEFT JOIN rr ON rr.delivery_line_id = l.id
  GROUP BY l.store_id, l.sku_id, l.delivered_on
), gaps AS (
  SELECT v.*, lead(v.delivered_on) OVER w AS nxt
  FROM visits v
  WINDOW w AS (PARTITION BY v.store_id, v.sku_id ORDER BY v.delivered_on)
), done AS (
  SELECT store_id, sku_id, delivered_on,
         nxt - delivered_on AS days_gap,
         GREATEST(q - ret, 0)::numeric / (nxt - delivered_on)::numeric AS rate,
         (ret = 0) AS sold_out,
         row_number() OVER (PARTITION BY store_id, sku_id ORDER BY delivered_on DESC) AS rn
  FROM gaps
  WHERE nxt IS NOT NULL AND nxt > delivered_on
), stats AS (
  SELECT store_id, sku_id,
         count(rate) AS visit_count,
  ...
  FROM visits ORDER BY store_id, sku_id, delivered_on DESC
)
SELECT l.store_id, l.sku_id, s.name AS store_name, sk.name AS sku_name,
       l.last_visit_date, l.last_delivered_qty, st.visit_count,
       ...
       CASE WHEN st.avg_rate > 0::numeric
            THEN l.last_visit_date + LEAST((sk.shelf_life_days - 1)::numeric, GREATEST(1::numeric, floor(l.last_delivered_qty::numeric / st.avg_rate)))::integer
